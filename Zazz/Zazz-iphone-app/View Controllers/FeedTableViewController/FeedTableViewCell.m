...
- (void)visualizeCellWithUserImage:(NSURL *)imageUrl {
    ...
    if (!imageUrl) return;
    [userImage downloadImageFromUrl:imageUrl];
}

- (void)visualizeCellWithContent:(NSURL *)imageUrl {
    /* Condition validation */
    if (!imageUrl) return;
    [feedCellContentView downloadImageFromUrl:imageUrl];
}
...
-(void)showImage:(UIButton*)imageButton{
    ...
    NSArray* objects  = [NSArray arrayWithObjects:
                         detailView,
                         [NSString stringWithFormat:@"detailView-tag%ld",(long)imageButton.tag],
                         nil];
